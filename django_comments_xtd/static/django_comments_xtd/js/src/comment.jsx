...
export class Comment extends React.Component {
  ...
  _get_right_div_chunk() {
    ...
    if(this.props.settings.is_authenticated &&
       (this.props.settings.can_moderate || current_user_id == this.props.data.user_id))
      {
        let remove_msg = django.gettext("remove comment");
        if(this.props.settings.can_moderate)
          url = this.props.settings.delete_url.replace('0', this.props.data.id);
        else
          url = this.props.settings.delete_url_own.replace('0', this.props.data.id);
  ...
  _get_feedback_chunk(dir) {
    ...
    let class_icon = "big-icon fas fa-"+icon;
  ...
  make_edit_form_invisible(submit_status) {
  this.setState({edit_form: {component: this.state.edit_form.component,
                             is_visible: false}});
  this.props.on_comment_created();
  }
  ...
  handle_reply_click(event) {
    ...
    if(this.state.edit_form.is_visible)
    {
      this.setState({edit_form: {component: this.state.edit_form.component,
                             is_visible: false}});
    }
  ...
  let component = this.state.edit_form.component;
  let visible = !this.state.edit_form.is_visible;
  if(component == null)
      component = (
        <CommentForm {...this.props.settings}
                     comment={this.props.data.comment}
                     comment_id={this.props.data.id}
                     on_comment_created={this.make_edit_form_invisible.bind(this)} />
      );
    this.setState({edit_form: {component: component, is_visible: visible}});
  }
  ...
  _get_edit_link_chunk () {
  let url = this.props.settings.edit_url.replace('0', this.props.data.id),
        edit_label = django.gettext("Edit");
   return (
        <span>&nbsp;&nbsp;<span className="text-muted">&bull;</span>&nbsp;&nbsp;
          <a className="small mutedlink" href={url}
             onClick={this.handle_edit_click}>{edit_label}</a>
        </span>
      );
  ...
  render_edit_form() {
    if(!this.state.edit_form.is_visible)
      return "";
    return (
      <div>{this.state.edit_form.component}</div>
    );
  }
  ...
  render() {
    ...
    if (!this.props.data.is_removed) {
      ...
      let current_user_id = this.state.current_user.split(":")[0];
      if(current_user_id == this.props.data.user_id)
      {
        edit_link = this._get_edit_link_chunk();
      ...
      <div className="media" id={comment_id}>
        ...
        <div className="media-body">
          <div className="comment pb-3">
            ...
            {comment_body}{feedback_btns}{reply_link}{edit_link}
            {reply_form}{edit_form}
          </div>
          {children}
        </div>
      </div>
